...
#Uw com
if [ $UW_COM == "True" ]; then
    SERIAL_PARSER_CMD="ros2 run serial_parser serial_parser --ros-args   -p port:=\"/dev/ttyUSB0\"   -p baudrate:=9600   -p listen_to_topic:=\"/evolo/sensors/succor/to\"   -p publish_topic:=\"/evolo/sensors/succor/from\""
